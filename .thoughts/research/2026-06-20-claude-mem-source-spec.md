...
## Zero-key holistic (Abu's requirement) — also verify the embedding path in 3B
MemWal `recallManual`/`rememberManual` need embeddings. CONFIRM whether the **MemWal relayer embeds server-side** (→ truly key-free) vs the manual client embedding client-side with `ONEMEM_EMBEDDING_API_KEY`. If relayer-side, eliminate the embedding key too. Don't assume — check the relayer + memwal client in 3B.
